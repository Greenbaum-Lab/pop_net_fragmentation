...
from distance_matrices import get_random_walk_matrix
from funcs import load_data
from mantel_test import calculate_mantel
...
def plot_mantel(df):
    ...
    plt.ylim(-0.05, 1.1)
    plt.savefig(f'./figs/cor_fst_euclidean.svg', format="svg")
    plt.show()
...
def plot_fst_distance(fragmentation_type, steps, dist_type:str, perms=999,  output_path='./figs/random_fst_steps.svg'):
    """
    Plot single correlation FST-distance for specified fragmentation types and steps.
    :param fragmentation_types: List of fragmentation types to process
    ...
    for i, step in enumerate(steps):
        ...
        distance_matrix = get_random_walk_matrix(net)
        r, p = calculate_mantel(net=net, fst_matrix=fst, dist_type=dist_type, perms=perms)
